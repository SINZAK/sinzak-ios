...
final class WorksSelectAlignVC: SZVC {
    ...
    private var alignInfoRelay: BehaviorRelay<(isEmployment: Bool, align: AlignOption)>//    private let refresh: () -> Void
    
    private let disposeBag = DisposeBag()
    
    // MARK: - Sections
    private let alignOptionSections: BehaviorRelay<[AlignOptionDataSection]> = BehaviorRelay(value: [
        AlignOptionDataSection(items: [AlignOption.recommend, AlignOption.recent].map {
    ...
    private lazy var alignOptionDataSource: RxTableViewSectionedReloadDataSource<AlignOptionDataSection> = RxTableViewSectionedReloadDataSource<AlignOptionDataSection>(
        configureCell: { [weak self] _, tableView, indexPath, item in
            ...
            if item.alignOption == self.alignInfoRelay.value.align {
    ...
    init(
        with alignInfoRelay: BehaviorRelay<(isEmployment: Bool, align: AlignOption)>
    ) {
...
private extension WorksSelectAlignVC {
...
extension WorksSelectAlignVC: UITableViewDelegate {
    func tableView(
...
private extension WorksSelectAlignVC {
    ...
    func selectAlignOption(with indexPath: IndexPath) {
        ...
        let isEmployment = alignInfoRelay.value.isEmployment
        self.alignInfoRelay.accept((isEmployment, selectedCell.alignOption ?? .recommend))
